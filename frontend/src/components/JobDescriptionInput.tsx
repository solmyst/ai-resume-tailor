...
import { Briefcase, Link, Sparkles, Target, AlertCircle } from 'lucide-react';
import { JobDescription } from '../App';
import { apiService, JobAnalysis } from '../services/api';
...
export function JobDescriptionInput({ onSubmit, isProcessing }: JobDescriptionInputProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Analyze job description using real API
      const analysis = await apiService.analyzeJob(jobText);
      ...
      const jobDescription: JobDescription = {
        text: jobText,
        extractedSkills: analysis.required_skills.concat(analysis.preferred_skills),
...
• Experience with microservices architecture`;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) => {
              ...
              setPreviewSkills(extractSkillsFromText(e.target.value));
              setError(null);
            }}
      ...
      {jobText.trim() && previewSkills.length > 0 && (
        <div className="mb-6 p-4 bg-slate-50 rounded-lg">
          <div className="flex items-center space-x-2 mb-2">
            <Target className="h-5 w-5 text-blue-600" />
            <span className="font-medium text-slate-700">Preview: Detected Skills</span>
          </div>
